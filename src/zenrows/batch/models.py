...
#   timestamp: 2026-09-24T07:40:10+00:00
...
class Run(BaseModel):
    ...
    status: RunStatus
    stats: RunStats
    duplicate_tasks: int | None = None
    """
    How many of the tasks this run was materialised from repeat
    a request an earlier task in the same list already makes.
    **Advisory — nothing is deduplicated.** Every task is
    scraped, charged and returned separately, and `stats.total`
    counts them all: three identical tasks are
    `total: 3, duplicate_tasks: 2`.

    Two tasks are the same request when `url`, the HTTP method,
    the body, and the merged `zenrows_params` (job-level
    overlaid with per-task) all match. `external_id` and
    `metadata` are excluded — they don't change what is fetched
    or what it costs. URLs are compared exactly: no trailing-
    slash, query-order, or encoding normalisation.

    `Idempotency-Key` does not cover this. It deduplicates
    whole submits, never URLs inside one.

    **`null` means not counted, which is different from `0`.**
    A large (202) submission returns before anything has walked
    the task list — counting there would put the cost on the
    request path, which is exactly what large uploads can't
    afford. The number lands with the `ingest_status: done`
    latch, so poll `GET /jobs/{id}` for it.

    Tasks added later via `addTasks` do not move this: that
    count is per-batch and is on the `addTasks` response.
...
class SubmitJobResponse(BaseModel):
    ...
    accepted_tasks: int
    """
    Every task accepted, duplicates included. The duplicate
    count is on `latest_run.duplicate_tasks` rather than here:
    a large (202) submission is answered before anything has
    walked the task list, so a top-level field would be null on
    exactly the submissions large enough to care.
...
class AddTasksResponse(BaseModel):
    accepted_tasks: int
    duplicate_tasks: int | None = 0
    """
    Always emitted by the current server; not `required`, for
    the same forward-compatibility reason as on
    `SubmitJobResponse`.

    Repeats **within this batch**, same rule as
    `SubmitJobResponse.duplicate_tasks`. Tasks already on the
    run are not consulted: a URL sent in an earlier batch and
    again in this one does not count.
