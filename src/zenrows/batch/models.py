...
#   timestamp: 2026-09-24T06:24:38+00:00
...
class SubmitJobResponse(BaseModel):
    ...
    accepted_tasks: int
    """
    Every task accepted, duplicates included.

    """
    duplicate_tasks: int | None = 0
    """
    Always emitted by the current server. Not `required`, so a
    client built against this spec keeps parsing responses from
    a server that predates the field.

    How many of the accepted tasks repeat a request an earlier
    task in the same submit already makes. **Advisory — nothing
    is deduplicated.** Each one is scraped, charged, and
    returned separately, so three identical tasks read as
    `accepted_tasks: 3, duplicate_tasks: 2`.
