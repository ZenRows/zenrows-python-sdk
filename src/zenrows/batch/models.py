...
#   timestamp: 2026-09-25T09:22:21+00:00
...
class AddTasksResponse(BaseModel):
    ...
    """
    Not `required`, so a client built against this spec keeps
    parsing responses from a server that predates the field.

    Repeats **within this batch**, same rule as
    `Run.duplicate_tasks`. Tasks already on the
